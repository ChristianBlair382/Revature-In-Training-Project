...
from app.orm_models.diagnostic_report import Diagnostic_Report
...
async def list_diagnostic_reports(db: AsyncSession = Depends(get_db)):
...
async def find_diagnostic_report_by_id(diagnostic_report_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_diagnostic_report(payload: Diagnostic_Report_Create, db: AsyncSession = Depends(get_db)):
