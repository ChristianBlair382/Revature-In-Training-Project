...
from app.orm_models.enums import SERVICE_CALL_PRIORITY
from app.orm_models.service_call import Service_Call
...
async def find_service_call_by_id(service_call_id: int, db: AsyncSession = Depends(get_db)):
...
async def colocation_discrepencies(priority: SERVICE_CALL_PRIORITY | None = None, db: AsyncSession = Depends(get_db)):
    statement = (
        select(
...
async def create_service_call(payload: Service_Call_Create, db: AsyncSession = Depends(get_db)):
